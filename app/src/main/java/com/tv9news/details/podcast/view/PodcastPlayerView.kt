...
class PodcastPlayerView : LinearLayout, View.OnClickListener, SeekBar.OnSeekBarChangeListener,
    ...
    private fun init() {
        ...
        if (myPlaylist!=null && myPlaylist.size>0 && !TextUtils.isEmpty(myPlaylist.get(0).article_title)) {
            txtCurrentMusic.text = myPlaylist.get(0).article_title
        }
        if (myPlaylist!=null && myPlaylist.size>0 && !TextUtils.isEmpty(myPlaylist.get(0).article_image)) {
            Helper.loadImage(mainImageView, myPlaylist.get(0).article_image?:" ", "")
        }
    ...
    fun initPlaylist(
        ...
        jcPlayerManager.playlist = playlist as ArrayList<Lists>
     ...
     */
    fun initAnonPlaylist(playlist: List<Lists>) {
     ...
     */
    fun initWithTitlePlaylist(playlist: List<Lists>, title: String) {
     ...
     */
    fun addAudio(jcAudio: Lists): Int {
...
//        }
    ...
    private fun onUpdateTitle(audio: Lists?) {
     ...
     * Sorts the playlist.
     */
    private fun sortPlaylist(playlist: List<Lists>) {
     ...
     */
    private fun isAlreadySorted(playlist: List<Lists>?): Boolean {
     ...
     */
    private fun generateTitleAudio(playlist: List<Lists>, title: String) {
        for (i in playlist.indices) {
